...
基于 TMDB 数据，对电影/剧集/番剧做批量重命名与目录整理，输出 Jellyfin 可直接识别的结构。**当前阶段：重命名与目录整理（P0）已完成；Bangumi 数据查询原语（P1）已就绪，但尚未接入 fetch-summary 交叉校验**（NFO 生成、Bangumi 双源填充、非标场景自动处理在后续阶段）。
