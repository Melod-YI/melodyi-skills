...
使用 click 实现命令行界面，支持：
- melodyi-search search <query> [--max-results] [--time-range] ...
- melodyi-search config show
- melodyi-search --version
"""
...
@click.group()
@click.version_option(version=__version__, prog_name="melodyi-search")
def cli():
...
def search(
    ...
    示例:
        melodyi-search search "Python教程"
        melodyi-search search "AI新闻" --max-results 20 --time-range day
        melodyi-search search "技术博客" --include-domains blog.csdn.net -i juejin.cn
    """
    try:
        # 1. 加载配置
...
def config_show(config_path: Optional[str], output: str):
    ...
    示例:
        melodyi-search config show
        melodyi-search config show --output json
    """
    try:
        config = load_config(config_path)
