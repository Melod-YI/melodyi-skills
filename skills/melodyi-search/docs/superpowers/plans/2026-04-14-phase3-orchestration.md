...
**目标:** 实现执行策略（正常模式串行回退 + 比对模式并发执行）、参数适配服务、回退重试机制，将提供商串联为完整的搜索流程。

**架构:** domain/services 层实现 ExecutionStrategy 和 ParameterAdapter，编排提供商调用。

**技术栈:** Python 3.10+, asyncio (并发执行), pytest-asyncio (异步测试)
...
class TestExecutionStrategy:
    ...
    @pytest.mark.asyncio
    async def test_execute_normal_returns_first_success(self):
        """测试正常模式返回第一个成功结果"""
        providers = [
    ...
    @pytest.mark.asyncio
    async def test_execute_normal_fallback_on_error(self):
        """测试正常模式错误时回退"""
        providers = [
    ...
    @pytest.mark.asyncio
    async def test_execute_comparison_returns_fastest(self):
        """测试比对模式返回最快结果"""
        providers = [
    ...
    @pytest.mark.asyncio
    async def test_execute_comparison_logs_all_results(self):
        """测试比对模式记录所有结果"""
        providers = [
            MockProvider("p1", delay_ms=100),
            MockProvider("p2", delay_ms=200),
        ]
        ...
        assert len(result.comparison_log) == 2

    @pytest.mark.asyncio
    async def test_execute_all_fail_returns_error(self):
        """测试所有提供商失败返回错误"""
        providers = [
...
class ExecutionStrategy:
    ...
    async def execute_comparison(
        ...
        Returns:
            UnifiedSearchResult: 最快的成功结果
        """
        start_time = time.time()

        # 并发执行所有提供商
        tasks = [self._execute_provider(provider, request) for provider in providers]

        # 使用 asyncio.wait 获取最快完成的结果
        ...
        for task in done:
            result = task.result()
            comparison_log[result.provider] = {
                "status": "success" if not result.error else "error",
                "time_ms": result.response_time_ms,
                "results_count": len(result.results),
                "error": result.error
        ...
        if self.logger:
            winner_name = winner_result.provider if winner_result else "none"
            self.logger.log_comparison_summary(winner_name, comparison_log)

        # 返回最快结果
        if winner_result:
            unified = self._convert_to_unified(winner_result)
            unified.comparison_log = comparison_log
            return unified
...
async def test_comparison_mode():
    ...
    assert result.comparison_log is not None
    assert len(result.comparison_log) == len(providers)

    print(f"比对模式测试完成")
    print(f"最快提供商: {result.provider}")
    for name, data in result.comparison_log.items():
        print(f"  {name}: {data['status']}, {data['time_ms']}ms, {data['results_count']} results")
```
...
- [x] ExecutionStrategy：正常模式（串行回退）和比对模式（并发执行）
- [x] 回退机制：提供商失败时自动切换到下一个
- [x] 比对模式日志：记录所有提供商执行结果