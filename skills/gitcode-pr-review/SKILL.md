...
- `git commit` 必须带 `-m`
- 私有仓克隆：用户应预先配置 git credential helper 或 SSH key，agent 直接 `git clone <url>` 即可。**禁止在 URL 内嵌 token**（`https://token@gitcode.com/...`）——token 由 `gitcode` CLI 内部管理，不应暴露给 agent 或落入命令行/git remote 配置
