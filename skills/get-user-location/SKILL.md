...
「查找设备」页面可能短时间内对 `reverseGeocode` 发起多次调用。脚本会静默期等待收集这些调用：**取时间最新的那次结果为准**。当多次调用的请求经纬度**不一致**时，仅在 `--verbose` 模式下才会在经纬度行之后追加一条警告，例如：
...
**未加 `--verbose` 时静默取最新结果，不输出该警告**（避免噪音；不一致信息仍会记录到日志，仅 verbose 模式可见）。多次调用但经纬度一致时同样静默取最新。此时如需查看每次调用的详情，加 `--verbose`（见下文）。
